    ...
    UUPSUpgradeable
{
    ...
    function initializeGovernor(address _aiNode, uint256 _scoreLimit, address _admin) external initializer {
    ...
    function syncGovernorPulse(uint256 proposalId, uint256 riskScore, uint256 nonce, bytes calldata signature)
        external
        nonReentrant
    {
        GovernorStorage storage $ = _getGovernorStorage();
        ...
        bytes32 msgHash = keccak256(abi.encodePacked(proposalId, riskScore, nonce, address(this), block.chainid))
