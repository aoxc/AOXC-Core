...
contract AOXCGovernorTest is Test {
    ...
    function setUp() public {
        ...
        bytes memory initData = abi.encodeWithSelector(
            AOXCGovernor.initializeGovernor.selector, 
            aiNode, 
            INITIAL_SCORE_LIMIT, 
            admin
        );
    ...
    function _getPulseSignature(uint256 propId, uint256 score, uint256 nonce) internal view returns (bytes memory) {
        ...
        bytes32 h = keccak256(
            abi.encodePacked(propId, score, nonce, address(governor), block.chainid)
    ...
    function test_Sovereign_Veto_Lifecycle() public {
        address[] memory t = new address[](1); t[0] = address(0x1);
        uint256[] memory v = new uint256[](1); v[0] = 0;
        bytes[] memory c = new bytes[](1); c[0] = hex"60";
    ...
    function test_RevertIf_NeuralIdentityIsForged() public {
        ...
        for (uint256 i = 0; i < 64; i++) { forgedSig[i] = 0x02; }
        forgedSig[64] = 0x1b; 
