...
contract AOXCBridgeTest is Test {
    ...
    function setUp() public {
        ...
        bytes memory initData =
            abi.encodeWithSelector(AOXCBridge.initializeBridge.selector, admin, aiNode, treasury, address(token));
     ...
     */
    function _generateSignature(address _actor, uint256 _amount, uint32 _srcChain, bytes32 _txId)
        internal
        view
        returns (bytes memory)
    {
        bytes32 msgHash = keccak256(abi.encode(_actor, _amount, _txId, _srcChain, address(bridge), block.chainid))
    ...
    function test_RevertIf_BridgeSignatureIsForged() public {
        ...
        bytes32 msgHash = keccak256(abi.encode(user, amount, transferId, SOURCE_CHAIN, address(bridge), block.chainid))
            .toEthSignedMessageHash();
    ...
    function test_RevertIf_TransferIDIsReused() public {
        ...
        // Second use (Replay)
        vm.expectRevert(abi.encodeWithSelector(AOXCErrors.AOXC_Neural_SignatureReused.selector, transferId));
        bridge.finalizeMigration(user, amount, SOURCE_CHAIN, transferId, validSig);
    }
}
